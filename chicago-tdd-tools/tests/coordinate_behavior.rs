...
fn test_infinity_blade_coordinate_system_not_in_combat() {
    ...
    assert_eq!(
        system.move_to_notation(&Command::AllocStat("health".to_string())),
        "alloc:health"
    );
}
...
fn test_infinity_blade_coordinate_system_in_combat() -> Result<()> {
    ...
    let expected_coord = format!(
        "b0:Full:{}:Full:ep1:{}:cT:cb0",
        expected_enemy_id, announced_attack
    );
    ...
    // Test notations
    assert_eq!(
        system.move_to_notation(&Command::Attack(AttackDir::Left)),
        "attack:left"
    );
    assert_eq!(system.move_to_notation(&Command::Parry), "parry");
    assert_eq!(
        system.move_to_notation(&Command::PerfectParry(AttackDir::Overhead)),
        "perfect_parry:overhead"
    );
    assert_eq!(
        system.move_to_notation(&Command::Magic(MagicType::Fire)),
        "magic:fire"
    );
