...
use genie_core::spec::{Bounds3D, HistoryEvent, Place, Vector3, WorldSpec};
...
fn should_capture_427_es3_metadata_in_receipt_chain() {
    ...
    event.details.insert(
        ...
        serde_json::Value::String("UE4.27-ES3".to_string()),
    );
    event.details.insert(
        "rendering_api".to_string(),
        serde_json::Value::String("WebGL-ES3".to_string()),
    );
    event.details.insert(
        "modification_intent".to_string(),
        serde_json::Value::String("Add factory floor".to_string()),
    );
    spec.history.push(event);

    // 2. Act
    ReceiptChainManager::generate_receipt_chain(&mut spec, secret_salt)
    ...
    tampered_spec.history[0].details.insert(
        "engine_version".to_string(),
        serde_json::Value::String("UE5.0".to_string()),
    );

    let is_valid_tampered = ReceiptChainManager::verify_receipt_chain(&tampered_spec, secret_salt);
    assert!(
        !is_valid_tampered,
        "Tampered metadata must fail verification"
    );
}
