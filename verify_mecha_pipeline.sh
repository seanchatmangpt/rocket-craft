#!/usr/bin/env bash
# verify_mecha_pipeline.sh — Stage 6: Serve real Brm.wasm + Playwright proof + receipt for Mecha Factory Walkthrough.
...
PORT=8080
RECEIPT="$CWD/pwa-staff/test-results/mecha-factory-playwright-receipt.json"
...
echo -e "${BOLD}${CYAN}  Rocket Craft — Mecha E2E Walkthrough Proof ${RESET}"
echo -e "${BOLD}${CYAN}============================================${RESET}"
...
("$CWD/tools/target/release/rocket-cmd" html5 serve --project Brm --port $PORT --dir "$SERVE_DIR" >/tmp/html5-server.log 2>&1) &
SERVER_PID=$!
...
log_info "[4/5] Running Playwright Mecha Walkthrough proof..."
...
(cd "$CWD/pwa-staff" && \
  ...
  npx playwright test tests-e2e/mecha_factory_walkthrough_projection.spec.ts \
...
if [ $PLAYWRIGHT_EXIT -eq 0 ]; then
  log_success "Mecha Walkthrough COMPLETE — real WebGL2 pipeline proven"
  echo -e "  WASM: $WASM_FILE"
  echo -e "  Receipt: $RECEIPT"
else
  log_error "Mecha Walkthrough FAILED — Playwright exit $PLAYWRIGHT_EXIT"
  exit 1
fi
