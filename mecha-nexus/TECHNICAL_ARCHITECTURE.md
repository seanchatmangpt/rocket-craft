# Mecha Nexus — Technical Architecture

**Engine:** Unreal Engine 4.24 (C++)
**Base Codebase:** `infinity-blade-4/Source/InfinityBlade4/`
**Extension Module:** `infinity-blade-4/Source/MechaNexus/`
**Date:** 2026-06-16
...
8. [MechKit AR Bridge Technical Spec](#8-gunpla-ar-bridge-technical-spec)
...
Mecha Nexus is built as a **strict extension layer** on top of the Infinity Blade 4 (IB4) codebase. No IB4 source files are modified. All new functionality lives in the `MechaNexus` Unreal module, which depends on `InfinityBlade4` as a module dependency declared in `MechaNexus.Build.cs`.

This ensures:
- IB4 combat feel (parry windows, combo chains, weapon hit detection) is preserved unmodified
- MechaNexus features are independently testable and removable
...
[InfinityBlade4]  (Core module — read-only, never modified)
  ...
  └── UI/IB4HUD
        ...
        v
[MechaNexus]  (Extension module — all new code lives here)
  ├── Core/{GNTypes, GNGameMode}
  ├── Suits/{GNMobileSuit, GNMechKitPart, GNMobileSuitRegistry}
  ...
  └── Monetization/{GNGachaSubsystem, GNBattlePassManager, GNCollabEventManager}
        |
        v
[MechaNexusServer]  (Dedicated server build — thin overlay, no rendering)
```

### Key Inheritance Principles

| IB4 Base Class | MechaNexus Subclass | Extension Purpose |
|---|---|---|
...
Not all MechaNexus systems map to an IB4 base. The following are **new `UActorComponent` additions** attached to the MechaNexus player character at spawn:
...
The Mobile Suit is the central equipment unit of Mecha Nexus. It replaces the IB4 sword/shield pairing as the dominant piece of equipment, inheriting from `UIB4EquipmentBase` to slot into the existing equipment binding and stat application pipeline.

### Class Hierarchy

```cpp
// infinity-blade-4/Source/MechaNexus/Suits/GNMobileSuit.h

UENUM(BlueprintType)
enum class EMechaSeries : uint8
{
...
class MECHANEXUS_API UGNMobileSuit : public UIB4EquipmentBase
{
    GENERATED_BODY()
...
public:
    ...
    // MechKit Builder: player-attached parts that modify FMobileSuitStats
    UPROPERTY(VisibleAnywhere, Category = "MechKit")
    TArray<UGNMechKitPart*> EquippedParts;

    // AR integration data — populated by UGNARBridgeSubsystem on scan
    UPROPERTY(VisibleAnywhere, Category = "AR")
    FMechaAR ARIntegrationData;
    ...
    void OnPartEquipped(UGNMechKitPart* NewPart, EPartSlot Slot);
...
```cpp
// infinity-blade-4/Source/MechaNexus/Suits/GNMechKitPart.h
...
UCLASS(BlueprintType)
class MECHANEXUS_API UGNMechKitPart : public UObject
{
    GENERATED_BODY()
...
Catalog entries (20 base suits):
  UC:             XM-01-2, Nu Mecha, Unicorn Mecha
  Wing:           Wing Zero (EW), Tallgeese III
  SEED:           Strike Mecha, Freedom Mecha
  DoubleOh:       00 Raiser, Exia
  IBO:            Barbatos Lupus Rex, Gusion Rebake Full City
  WitchFromMercury: Mecha Aerial, Mecha Pharact
  TurnA:          Turn A Mecha, Turn X

  AR-unlock only: PG Unleashed XM-01-2, PG Unleashed Nu Mecha,
...
```cpp
// infinity-blade-4/Source/MechaNexus/SpecialSystems/GNSpecialSystem.h

UCLASS(Abstract, BlueprintType)
class MECHANEXUS_API UGNSpecialSystem : public UActorComponent
{
    GENERATED_BODY()
...
#### UGNFinFunnelSystem (Nu Mecha — UC)
...
#### UGNStrikerPackSystem (Strike Mecha — SEED)
...
#### UGNGUNDFormatSystem (Mecha Aerial — WitchFromMercury)
...
#### UGNMoonlightButterflySystem (Turn A Mecha — TurnA)
...
Extends IB4's combo tracking with Mecha-specific resource meters.

```cpp
// infinity-blade-4/Source/MechaNexus/Combat/GNComboComponent.h

UCLASS()
class MECHANEXUS_API UGNComboComponent : public UIB4CombatComponent
{
    GENERATED_BODY()
...
```cpp
// infinity-blade-4/Source/MechaNexus/Combat/GNParrySystem.h

UCLASS()
class MECHANEXUS_API UGNParrySystem : public UIB4ParrySystem
{
    GENERATED_BODY()
...
```cpp
// infinity-blade-4/Source/MechaNexus/Combat/GNMobileArsenal.h
...
Enemy pilots are distinct from IB4 Titans. Where IB4 enemies use `ETitanType` to drive 3-phase AI, Mecha Nexus pilots use `EGNPilotPersonality` which affects telegraphing, bluffing, and aggression curves across all 3 inherited phases.

```cpp
// infinity-blade-4/Source/MechaNexus/AI/GNPilotAI.h
...
UCLASS()
class MECHANEXUS_API UGNPilotAI : public AIB4EnemyBase
{
    GENERATED_BODY()
...
```cpp
// infinity-blade-4/Source/MechaNexus/Progression/GNNewtypeGauge.h

UCLASS()
class MECHANEXUS_API UGNNewtypeGauge : public UActorComponent
{
    GENERATED_BODY()
...
|---|---|---|
| >= 50% | Nu Mecha Fin Funnel passive activates (orbit mode) | Nu Mecha equipped |
| >= 50% | NT resonance flash activates on `UGNParrySystem` | Any UC-series suit |
| >= 80% | Amuro Ray Full Burst available | Nu Mecha equipped |
...
| == 100% | I-Field barrier (3 blocks) OR Newtype Awakening | Nu Mecha or Unicorn |
...
```cpp
// infinity-blade-4/Source/MechaNexus/Arena/GNDuelArena.h
...
enum class EGNDuelRuleSet : uint8
{
    ...
    CanonicalOnly     UMETA(DisplayName = "Canonical — no MechKit Builder parts"),
...
UCLASS()
class MECHANEXUS_API AGNDuelArena : public AGameModeBase
{
    GENERATED_BODY()
...
Backend: Supabase table `duel_challenges` with realtime subscription.
  Challenge flow:
    ...
    3. Client B accepts → dedicated server slot allocated (MechaNexusServer build)
...
```cpp
// infinity-blade-4/Source/MechaNexus/Arena/GNSpectatorComponent.h

UCLASS()
class MECHANEXUS_API UGNSpectatorComponent : public UActorComponent
{
    GENERATED_BODY()
...
## 8. MechKit AR Bridge Technical Spec

The AR Bridge connects physical MechKit kit purchases to in-game mobile suit unlocks. The system operates through barcode scanning (all kits) and NFC reading (PG Unleashed exclusively).
...
```cpp
// infinity-blade-4/Source/MechaNexus/AR/GNARBridgeSubsystem.h

UCLASS()
class MECHANEXUS_API UGNARBridgeSubsystem : public UGameInstanceSubsystem
{
    GENERATED_BODY()
...
```cpp
// infinity-blade-4/Source/MechaNexus/Progression/GNPilotProgression.h
...
UCLASS()
class MECHANEXUS_API UGNPilotProgression : public UIB4XPSystem
{
    GENERATED_BODY()
...
public:
    ...
    UPROPERTY(SaveGame) TMap<EMechaSeries, int32> SeriesELO;
...
"Black History Echo" is the Mecha Nexus name for New Game+, referencing Turn A's Black History records of past civilizations.
...
```cpp
// infinity-blade-4/Source/MechaNexus/Monetization/GNGachaSubsystem.h

UCLASS()
class MECHANEXUS_API UGNGachaSubsystem : public UGameInstanceSubsystem
{
    GENERATED_BODY()
...
```cpp
// infinity-blade-4/Source/MechaNexus/Monetization/GNBattlePassManager.h
...
UCLASS()
class MECHANEXUS_API UGNBattlePassManager : public UGameInstanceSubsystem
{
    GENERATED_BODY()
...
```cpp
// infinity-blade-4/Source/MechaNexus/Monetization/GNCollabEventManager.h

UCLASS()
class MECHANEXUS_API UGNCollabEventManager : public UGameInstanceSubsystem
{
    GENERATED_BODY()
...
Keyboard/Mouse gesture emulation:
  ...
  Tab:         Striker Pack swap (Strike Mecha only)
...
UCLASS()
class MECHANEXUS_API UGNSaveGame : public USaveGame
{
    GENERATED_BODY()
...
```
infinity-blade-4/Source/MechaNexus/
├── MechaNexus.Build.cs                  ← Module definition; depends on InfinityBlade4
│
├── Core/
│   ├── GNTypes.h                         ← EMechaSeries, EGNDuelRuleSet, FMobileSuitStats,
...
│   ├── GNMechKitPart.h / GNMechKitPart.cpp         ← UObject; stat modifier pieces
...
│   ├── GNFinFunnelSystem.h / .cpp                ← Nu Mecha; I-Field; Full Burst
...
### MechaNexus.Build.cs (summary)

```csharp
// infinity-blade-4/Source/MechaNexus/MechaNexus.Build.cs
...
MechKit Builder part combinations: stat aggregation is O(6) (max 6 part slots);
...
```
MechaNexus           — Mobile shipping binary (iOS + Android)
                        Includes: MechaNexus + InfinityBlade4 modules
                        Excludes: Editor tooling, server-only code
                        Packaging: UE4 Project Launcher; 
                        iOS: Xcode archive; Android: Gradle APK/AAB

MechaNexusEditor     — UE4 Editor build for content creation
                        Includes: MechaNexusEditor module (DataTable tools,
...
MechaNexusServer     — Dedicated server for Duel Arena
...
MechaNexusPC         — Steam desktop binary
                        Built from same source as MechaNexus mobile;
...
Build pipeline (GitHub Actions, referencing .github/workflows/ in rocket-craft repo):
  1. On push to `claude/master-integration`:
       a. Compile check: UnrealBuildTool MechaNexus (Editor target, Linux)
       b. Run unit tests: UE4 Automation Framework tests in MechaNexus/Tests/
       c. Static analysis: clang-tidy on MechaNexus Source/ 
  2. On tag `release/*`:
       a. Full cook + package: MechaNexus (iOS + Android)
       b. Package: MechaNexusServer (Linux)
       c. Upload: TestFlight (iOS) + Firebase App Distribution (Android)
  3. On merge to main:
       a. Deploy: MechaNexusServer containers via Rocket SDK deployment tooling
       b. Notify: Supabase config updated with new server endpoint

Module boundary enforcement:
  InfinityBlade4 module has no dependency on MechaNexus.
  If any InfinityBlade4 header includes a MechaNexus header, CI fails.
...
*Maintained by: Mecha Nexus engineering team*
*IB4 base codebase: `infinity-blade-4/Source/InfinityBlade4/` (read-only)*
*GN extension module: `infinity-blade-4/Source/MechaNexus/` (all new code)*
