...
${suffix}`;
  ...
  `;
    ...
    toggleBtn.textContent = "\u{1F6E0}\uFE0F HUD";
    ...
    function decodeJWT2(token) {
      try {
        ...
        return {
          ...
          exp: decoded.exp ? new Date(decoded.exp * 1e3).toLocaleString() : "N/A"
        };
      } catch (error) {
        logToConsole(
    ...
    async function updateAuthState(session) {
      ...
      const decoded = decodeJWT2(session.access_token);
      if (!decoded) {
    ...
    async function fetchDbStats() {
      try {
        ...
        const { count: playersCount, error: playersErr } = await supabase.from("players").select("*", { count: "exact", head: true });
        if (playersErr) throw playersErr;
        playersVal.textContent = playersCount !== null ? playersCount.toString() : "0";
        const { count: sessionsCount, error: sessionsErr } = await supabase.from("game_sessions").select("*", { count: "exact", head: true });
    ...
    async function submitMockScore() {
      try {
        ...
        const score = Math.floor(Math.random() * 501) + 500;
    ...
    fetchDbStats();
    window.addEventListener("game-score-update", async (e) => {
      const event = e;
