...
${suffix}`;
  ...
  var gameStateViewer = document.getElementById("game-state-viewer");
  ...
  function renderGameSessions(gameSessions) {
    if (!gameStateViewer) {
      return;
    }
    ...
    gameSessions.forEach((session) => {
      ...
      headers.forEach((header) => {
        ...
        cell.textContent = typeof cellValue === "object" ? JSON.stringify(cellValue, null, 2) : cellValue;
  ...
  async function getPlayer(id) {
    const { data, error } = await supabase.from("players").select("id, name, email").eq("id", id).single();
    if (error) {
      throw error;
    }
    return data;
    ...
    if (target.classList.contains("edit-button")) {
      ...
      if (playerId) {
        try {
          ...
          const playerEmailInput = document.getElementById("edit-player-email");
  ...
  async function handleEditFormSubmit(event) {
    ...
    if (playerIdInput && playerNameInput && playerEmailInput) {
      try {
        const { error } = await supabase.from("players").update({ name: playerNameInput.value, email: playerEmailInput.value }).eq("id", playerIdInput.value);
        if (error) {
  ...
  async function init() {
    ...
    try {
      ...
      if (gameStateViewer) {
        gameStateViewer.innerHTML = "<p>Error fetching game sessions. Please check the console for details.</p>";
      }
    }
  }
