...
pub fn run_inspect(file_path: Option<&str>, summary_only: bool) -> Result<()> {
    ...
    println!(
        "{}",
        "Scanning workspace for saved states and receipts...".dimmed()
    );
    let classified = scan_workspace(&root_dir);
    ...
    loop {
        ...
        let mut selections: Vec<String> = flat_list
            .iter()
...
fn print_header() {
    ...
    println!(
        "{}",
        "║             ROCKET-CRAFT DEVELOPER INSPECTOR             ║"
            .bold()
            .cyan()
    );
    println!(
        "{}",
        "║     MUD States  •  Receipts Chain  •  OCEL Event Trails  ║"
            .bold()
            .cyan()
    );
    println!(
...
fn classify_file(path: &Path) -> Option<String> {
    ...
    if obj.contains_key("visualDelta")
        && (obj.contains_key("prompt") || obj.contains_key("screenshots"))
    {
    ...
    if obj.contains_key("checkpoint")
        ...
        && obj.contains_key("digest_algorithm")
    {
...
fn print_mud_state(rel_path: &str, data: &Value) {
    ...
    println!(
        "\n{}",
        format!("Inspection: MUD Saved State ({})", rel_path)
            .bold()
            .green()
    );
    println!(
    ...
    println!(
        "   Level:     {}  |  XP: {}",
        player.get("level").unwrap_or(&Value::from(1)),
        player.get("xp").unwrap_or(&Value::from(0))
    );
    println!(
        "   Bloodline: {} (Rebirths: {})",
        player.get("bloodline").unwrap_or(&Value::from(0)),
        data.get("bloodline").unwrap_or(&Value::from(0))
    );
    println!(
        "   HP:        {} / {}",
        player.get("health").unwrap_or(&Value::from(0.0)),
        player.get("max_health").unwrap_or(&Value::from(0.0))
    );
    println!(
        "   Mana:      {} / {}",
        player.get("mana").unwrap_or(&Value::from(0.0)),
        player.get("max_mana").unwrap_or(&Value::from(0.0))
    );
    println!(
        "   Gold:      {}g",
        player
    ...
    println!(
        "   Attack:  {:<4} |   Defense: {}",
        player.get("stat_attack").unwrap_or(&Value::from(0)),
        player.get("stat_defense").unwrap_or(&Value::from(0))
    );
    println!(
        "   Magic:   {:<4} |   Health:  {}",
        player.get("stat_magic").unwrap_or(&Value::from(0)),
        player.get("stat_health").unwrap_or(&Value::from(0))
    );
    println!(
        "   Unallocated Stat Points: {}",
        player.get("stat_points").unwrap_or(&Value::from(0))
    );
    println!(
        "{}",
        "------------------------------------------------------------".dimmed()
    ...
    let queue_str = queue
        .map(|arr| {
            arr.iter()
                .map(|v| format!("{}", v.as_str().unwrap_or("").red()))
                .collect::<Vec<_>>()
                .join(" ➔ ")
        })
        .unwrap_or_else(|| "None".to_string());
...
fn print_verification_receipt(rel_path: &str, data: &Value) {
    ...
    println!(
        "\n{}",
        format!("Inspection: E2E Visual Verification Receipt ({})", rel_path)
            .bold()
            .green()
    );
    println!(
    ...
    let trace_str = trace
        .map(|arr| {
            arr.iter()
                .map(|v| format!("{}", v.as_str().unwrap_or("").bold()))
                .collect::<Vec<_>>()
                .join(" ➔ ")
        })
        .unwrap_or_else(|| "None".to_string());
    ...
    if let Some(arr) = logs {
        if arr.is_empty() {
            ...
            println!(
                "   Found {} console messages. Press 'l' inside REPL to list them.",
                arr.len()
            );
        }
    } else {
...
fn print_audit_receipt(rel_path: &str, data: &Value) {
    println!(
        "\n{}",
        format!("Inspection: GGen Audit / Affidavit Receipt ({})", rel_path)
            .bold()
            .green()
    );
    println!(
    ...
    println!(
        " Format Version: {}",
        data.get("format_version").unwrap_or(&Value::from("N/A"))
    );
    println!(
        " Chain Hash:     {}",
        data.get("chain_hash")
    ...
    if let Some(events) = data.get("events").and_then(|v| v.as_array()) {
        for ev in events {
            ...
            println!(
                "      ID:     {}",
                ev.get("id").and_then(|v| v.as_str()).unwrap_or("N/A")
            );
            if let Some(objs) = ev.get("objects").and_then(|v| v.as_array()) {
                let obj_strs: Vec<String> = objs
                    .iter()
                    .map(|obj| {
                        format!(
                            "{} ({}: {})",
...
fn print_sync_receipt(rel_path: &str, data: &Value) {
    println!(
        "\n{}",
        format!("Inspection: GGen Sync Receipt ({})", rel_path)
            .bold()
            .green()
    );
    println!(
    ...
    println!(
        " Prev Receipt:   {}",
        data.get("previous_receipt_hash")
            .and_then(|v| v.as_str())
            .unwrap_or("N/A")
    );
    println!(
...
fn print_compliance_receipt(rel_path: &str, data: &Value) {
    println!(
        "\n{}",
        format!("Inspection: OCEL Compliance Receipt ({})", rel_path)
            .bold()
            .green()
    );
    println!(
...
fn print_ocel_log(rel_path: &str, data: &Value) {
    println!(
        "\n{}",
        format!("Inspection: OCEL Event Log / Event Trail ({})", rel_path)
            .bold()
            .green()
    );
    println!(
    ...
    if let Some(event_types) = data.get("event_types").and_then(|v| v.as_array()) {
        let et_names: Vec<&str> = event_types
            .iter()
            .filter_map(|et| et.get("name").and_then(|n| n.as_str()))
            .collect();
...
fn inspect_file(filepath: &Path, rel_path: &str) -> Result<()> {
    let category = classify_file(filepath)
        .ok_or_else(|| anyhow!("Could not recognize JSON schema for file: {}", rel_path))?;
    ...
    loop {
        ...
        if opt == "q" || opt == "quit" || opt == "exit" {
            ...
            && category == VERIFICATION_RECEIPT
            && data.get("consoleLogs").is_some()
        {
