...
fn do_wait_port(port: u16, host: String, timeout_secs: u64) -> Result<Value> {
    ...
    eprintln!("Waiting for {}:{} (timeout {}s)...", host, port, timeout_secs);
    loop {
        ...
        if Instant::now() >= deadline {
            return Err(clap_noun_verb::NounVerbError::execution_error(
                format!("Timed out after {}s waiting for {}", timeout_secs, addr)
            ));
        }
...
fn wait_port(port: u16, host: Option<String>, timeout: Option<u64>) -> Result<Value> {
    do_wait_port(port, host.unwrap_or_else(|| "127.0.0.1".to_string()), timeout.unwrap_or(60))
}
