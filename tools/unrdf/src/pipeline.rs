    ...
    pub fn render(&self) -> Result<PipelineOutput, PipelineError> {
        ...
        for type_triple in &type_triples {
            ...
            for st in subject_triples {
                ...
                let raw_prop_name = raw_prop_name.split('#').next_back().unwrap_or(raw_prop_name);
                let raw_prop_name = raw_prop_name.split(':').next_back().unwrap_or(raw_prop_name);
                    ...
                    if props.is_empty() {
                        ...
                        for (prop_name, datatype) in &props {
                            ...
                            let rust_type = match datatype.as_deref() {
                                Some("xsd:integer") | Some("integer") | Some("xsd:int") | Some("int") => "i32",
                                Some("xsd:boolean") | Some("boolean") => "bool",
                                Some("xsd:float") | Some("float") | Some("xsd:double") | Some("double") => "f64",
                                _ => "String",
                            };
                    ...
                    if props.is_empty() {
                        ...
                        for (prop_name, datatype) in &props {
                            ...
                            let go_type = match datatype.as_deref() {
                                Some("xsd:integer") | Some("integer") | Some("xsd:int") | Some("int") => "int",
                                Some("xsd:boolean") | Some("boolean") => "bool",
                                Some("xsd:float") | Some("float") | Some("xsd:double") | Some("double") => "float64",
                                _ => "string",
                            };
                    ...
                    if props.is_empty() {
                        ...
                        for (prop_name, datatype) in &props {
                            ...
                            let ts_type = match datatype.as_deref() {
                                Some("xsd:integer") | Some("integer") | Some("xsd:int") | Some("int") => "number",
                                Some("xsd:boolean") | Some("boolean") => "boolean",
                                Some("xsd:float") | Some("float") | Some("xsd:double") | Some("double") => "number",
                                _ => "string",
                            };
                            fields.push_str(&format!("    {}: {};\n", clean_name, ts_type));
                        }
...
mod tests {
    ...
    fn test_pipeline_render_with_properties_rust() {
        ...
        pipeline.load_turtle("<http://example.org/Character> rdf:type <http://schema.org/Thing> .").unwrap();
        
        // Add string property
        pipeline.store.add(Triple::new("http://example.org/Character", "http://example.org/name", Term::Literal {
            value: "Hero".into(),
        ...
        pipeline.store.add(Triple::new("http://example.org/Character", "http://example.org/age", Term::Literal {
            value: "25".into(),
