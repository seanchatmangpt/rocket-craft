//! mecha_mech_character — Mecha-themed UE4 Character Blueprint
//!
//! Generates a MechaMechCharacter Blueprint with:
//!   - A "MechaMesh" string variable (proxy for SkeletalMeshComponent ref)
...
//!
//! Run: cargo run --example mecha_mech_character -p blueprint-core > /tmp/MechaMechCharacter.T3D
...
fn main() {
    ...
    // -----------------------------------------------------------------------
    let mut b = BlueprintBuilder::new("MechaMechCharacter", "Character");

    // ------------------------------------------------------------------
    // Variables
    // ------------------------------------------------------------------
    // MechaMesh — declared as a String variable; in UE4 set the type to
    // SkeletalMeshComponent in the Blueprint editor after import.
    b.add_variable_mut("MechaMesh", VarType::String, Some("None".to_string()));
    ...
    //   BeginPlay → "Mecha systems online!" → "Cockpit pressurised"
    // ------------------------------------------------------------------
    let begin_play = b.begin_play_node();
    let init_msg = b.print_string("Mecha systems online! XM-01-2 initiating...");
    b.exec_connect(&begin_play, &init_msg);
    let cockpit_msg = b.print_string("MechaMech: Cockpit pressurised. Pilot ready for launch.");
    ...
    let tick_msg = b.print_string("[MechaMech] Tick: all systems nominal.");
    ...
    let destroyed_msg = b.print_string("CRITICAL — MechaMech destroyed! Ejecting pilot...");
...

    {
        ...
        let mut set_saber_on =
            nodes::set_variable("SetBeamSaberActive_On", "BeamSaberActive", PinType::bool())
                .at(2000, 200);
        ...
        let mut set_saber_off =
            nodes::set_variable("SetBeamSaberActive_Off", "BeamSaberActive", PinType::bool())
                .at(2000, 500);
