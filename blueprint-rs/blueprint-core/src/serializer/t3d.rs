...
mod tests {
    ...
    #[test]
    fn empty_blueprint_serializes_without_panic() {
        let bp = Blueprint::new("EmptyActor", "AActor");
        let t3d = T3dSerializer::serialize(&bp);
        assert!(!t3d.is_empty(), "serialized output must not be empty");
    }

    #[test]
    fn serialize_with_header_contains_generated_comment() {
        let bp = Blueprint::new("GeneratedBP", "Pawn");
        let t3d = T3dSerializer::serialize_with_header(&bp);
        assert!(t3d.contains("// Generated by blueprint-rs"));
        assert!(t3d.contains("GeneratedBP"));
        assert!(t3d.contains("Pawn"));
    }

    #[test]
    fn hidden_pin_has_bhidden_attribute() {
        let mut pin = Pin::new(
            "OutputDelegate",
            PinDirection::Output,
            PinType {
        ...
        let t3d = serialize_node(&node);
        assert!(
            t3d.contains("bHidden=True"),
            "hidden pin must have bHidden attribute, got:\n{}",
            t3d
        );
    }

    #[test]
    fn orphaned_pin_has_borphanedpin_attribute() {
        let mut pin = Pin::data_input("OldPin", PinType::string());
        pin.is_orphaned = true;
        ...
        let t3d = serialize_node(&node);
        assert!(
            t3d.contains("bOrphanedPin=True"),
            "orphaned pin must have bOrphanedPin attribute, got:\n{}",
            t3d
        );
    }

    #[test]
    fn node_position_emitted_in_output() {
        let node = BpNode::new(
            "/Script/BlueprintGraph.K2Node_CallFunction",
        ...
        let t3d = serialize_node(&node);
        assert!(t3d.contains("NodePosX=256"), "X position missing: {t3d}");
        assert!(t3d.contains("NodePosY=128"), "Y position missing: {t3d}");
    }
}
