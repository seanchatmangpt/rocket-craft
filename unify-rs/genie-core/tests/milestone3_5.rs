...
fn test_milestone3_layout_compiler() {
    ...
    let bounds = Bounds3D::new(Vector3::new(10.0, 20.0, 30.0), Vector3::new(50.0, 50.0, 50.0));
    ...
    assert!(t3d.contains("Begin Map"), "T3D map does not contain 'Begin Map'");
    assert!(t3d.contains("Begin Level"), "T3D map does not contain 'Begin Level'");
    assert!(t3d.contains("Place_room_1"), "T3D map does not contain place room_1");
    assert!(t3d.contains("Actor_bot_1"), "T3D map does not contain actor bot_1");
    assert!(t3d.contains("Object_cnc_1"), "T3D map does not contain object cnc_1");
    assert!(t3d.contains("Cube.Cube"), "T3D map does not contain Cube mesh reference");
    assert!(t3d.contains("Cylinder.Cylinder"), "T3D map does not contain Cylinder mesh reference");
    assert!(t3d.contains("Sphere.Sphere"), "T3D map does not contain Sphere mesh reference");
    assert!(t3d.contains("End Level"), "T3D map does not contain 'End Level'");
    assert!(t3d.contains("End Map"), "T3D map does not contain 'End Map'");
}
...
fn test_milestone4_deployment_manager() {
    ...
    assert!(log_content.contains("Place: room_1"), "Log missing Place info");
...
fn test_milestone5_world_evolver() {
    ...
    assert_eq!(evolved_bot.placement.position, Vector3::new(10.0, 20.0, 30.0));
    assert_eq!(evolved_bot.placement.rotation, Vector3::new(0.0, 180.0, 0.0));

    // Verify history events
    assert_eq!(evolved.history.len(), 1, "Expected 1 history event from evolution");
    assert_eq!(evolved.history[0].activity, "Evolve");
    assert!(evolved.history[0].details.contains_key("modification_intent"));

    // Verify receipt chain
    assert!(!evolved.receipts.is_empty(), "Expected receipts to be generated");
