...
impl ReceiptChainManager {
    ...
    pub fn generate_receipt_chain(spec: &mut WorldSpec, secret_salt: &[u8]) -> Result<(), GenieError> {
        ...
        for (idx, event) in sorted_history.iter().enumerate() {
            let event_bytes = serde_json::to_vec(event)
                .map_err(|e| GenieError::Evolution(format!("Failed to serialize history event: {}", e)))?;
            ...
            let receipt = if idx == 0 {
                ...
                let mut data = Vec::with_capacity(secret_salt.len() + engine_bytes.len() + event_bytes.len());
                ...
                let mut data = Vec::with_capacity(secret_salt.len() + prev_hash.len() + event_bytes.len());
                data.extend_from_slice(secret_salt);
