# Gap Analysis Report — GC-MECHA-FACTORY-001

## Status: PARTIAL
**Object under test:** Mecha Factory Walkthrough & Packaging Flow
**Observed evidence:** 
- `crates/rocket_preue4_verifier` contains hardcoded MechBirth dependencies.
- `ggen-validation-tests/ggen.toml` lacks generation rules for Mecha Factory deliverables.
- `verify_html5_pipeline.sh` is tailored specifically for the Brm game target.
- Playwright tests inside `pwa-staff/tests-e2e` do not yet target Mecha Factory.
...
- **Refactoring CLI Entrypoint:** Modify `rocket_preue4_verify.rs` to support a `--milestone` or `--config` flag to load different steps and expected artifact arrays (e.g. Mecha Factory).
- **Mecha Factory Step Definitions:** Add the Mecha Factory walkthrough steps to the verifier (e.g., source admission, frame selection, physics handler validation, movement injection, visual projection, receipt emission).
- **New Integration Tests:** Create `tests/integration_mecha_factory.rs` and `tests/chaos_mecha_factory.rs` loading the new walkthrough's event trace instead of `out.json`.

---

## 2. procedurally Generating Mecha Factory Deliverables with `ggen`

### Observations:
- **Ontologies:** 
  - `ontology/mecha_nexus.ttl` contains mech structural primitives (`gn:Frame`, `gn:Mobility`, `gn:Power`, etc.) and specific mobile suit components (`gn:MechaFrame`, `gn:MinovskyUltracompact`).
  - `ggen-validation-tests/core.ttl` defines character instances (`mecha:MyMecha`), input/movement graph topology (`mecha:MechaInputGraph`), and typestates.
- **Configuration:** `ggen-validation-tests/ggen.toml` does not define generation rules for the Mecha Factory walkthrough files. It only contains validation rules (`R1`-`R4`, `RuleA`-`RuleM`) and a single stub `readme` generation rule.
...
- **`MechaFactorySteps.h` / `MechaFactorySteps.rs`:** SPARQL queries to extract the sequence of states/steps (e.g. `mecha:MechaInputGraph` node connections) and format them as C++ and Rust enums.
- **`MechaFactoryWalkthrough.csv`:** Extract pin connection values and movement vectors for input actuation from the RDF graph.
- **`MechaFactorySocketTopology.csv`:** Query components attached via `ue4:hasComponent` and output their mounting locations.
- **`MechaFactorySkinLayers.csv`:** Query materials and armor layers (`gn:Armor` subclasses/instances) and output their thickness and mass.
- **`MechaFactoryMotionFamilies.csv`:** Extract mobility profiles (`gn:Mobility` instances like walking, hover) and speed limits.
- **`MechaFactorySemanticLOD.csv`:** Query LOD allocations (`ue4:semantic_lod_class` properties).
- **`MechaFactoryAuthority.h`:** Map replicated fields (`ue4:bReplicates true` attributes on properties).
- **`MechaFactoryTypestates.h`:** Formulate valid state transitions based on `ue4:CookingTypestate` and `ue4:WasmPackagingTypestate` mappings.
...
### Gaps & Required Changes:
- To support `GC-MECHA-FACTORY-001`, the generated files under `generated/mecha_factory/` must be copied to the `versions/v4_27_0/Source/` and `Content/` directories prior to building.
- The uproject map redirect (`DefaultEngine.ini`) must point the startup map to the Mecha Factory walkthrough level (e.g. `/Game/Maps/MechaFactory`).
- Extend `Html5Cook::run` in `tools/rocket-sdk/src/html5.rs` to support variable project builds so that a Mecha Factory build command can copy the correct generated artifacts automatically before UAT compiles them.
...
### Gaps & Required Changes:
- **New Test Spec:** Implement `mecha_factory_walkthrough_projection.spec.ts` matching the actuation pattern for Mecha walkthrough (e.g., injecting `W` key to walk the Mecha forward).
- **Config Addition:** Add `mecha_factory_walkthrough_projection.spec.ts` to `playwright.html5.config.ts` matches list or pass it directly.
- **Receipt Writing:** Write the resulting receipt with `verdict`, `visualDelta`, and `output_hash` (WASM SHA256) to `pwa-staff/test-results/mecha-factory-receipt.json`.
- **WASM Mapping:** Package and host `MechaFactory.html`/`MechaFactory.wasm` in `pwa-staff/manufactured/`.
