...
3. **Scripts & Tests**:
   ...
   - `pwa-staff/tests-e2e/mecha_factory_walkthrough_projection.spec.ts` (Line 193) reports a failed movement check where `numDiffPixels` is `55` against an expected threshold of `70` (resulting in a test exit code of `1`).
...
4. **Offline Test Consistency**: Standalone runs of `mecha_offline.test.ts` pass, but a full build is blocked by the Mecha walkthrough's slow camera displacement (55px delta vs 70px target) and general HTML5 blake3 ESM exports. Resolving the Mecha walkthrough require increasing character velocity or camera placement, and updating blake3 import paths to use `.js` extension.
