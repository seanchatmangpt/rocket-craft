...
1. **Test Infrastructure Documentation (`/Users/sac/rocket-craft/TEST_INFRA.md`):**
   - Lines 130-137 define the "Mecha Player Character Scenario" under Tier 4:
     ```markdown
     130: 1. **Case 4.1: The Mecha Player Character Scenario**
     131:    - Scenario: Define a `ue4:ACharacter` subclass representing a Mecha. It contains:
     ...
     137:    - Verification: SPARQL queries must verify that all parts of the Mecha character are structurally and logically connected without dangling links.
     ```
...
2. **Validation Harness Scripts (`/Users/sac/rocket-craft/validate_ontology.sh` and `/Users/sac/rocket-craft/ggen-validation-tests/verify_all_rules.sh`):**
   - Running the default production pack validation via `/Users/sac/rocket-craft/validate_ontology.sh`:
     ...
     ```
   - Running the rule verification suite `/Users/sac/rocket-craft/ggen-validation-tests/verify_all_rules.sh` (task-43) which tests all 16 custom validation rules and SHACL shapes against the Mecha/test ontologies:
     ```bash
...
3. **Mecha Ontology Verification in Production Pack:**
...
1. **Tier 4 Scenario Alignment:** The definition of the Mecha Player Character Scenario in `TEST_INFRA.md` requires that `ue4:ACharacter` contains components, graphs, subsystem handlers, and typestates correctly linked together.
2. **Implementation Verification:** We viewed `/Users/sac/rocket-craft/ggen-validation-tests/core.ttl` (lines 167-304) and confirmed that:
   - `mecha:MyMecha` is defined as `mecha:AMechaCharacter` (subclass of `ue4:ACharacter`).
   - It possesses components `mecha:MechaMesh` (`ue4:USkeletalMeshComponent`) and `mecha:MechaCollision` (`ue4:UBoxComponent`).
   - It references `mecha:MechaInputGraph` (`ue4:UEdGraph`) containing keyboard input events (`mecha:W_KeyPressedNode`) connected to the movement function call (`mecha:MoveForwardCallNode`).
   - It declares `ue4:bReplicates true` and `ue4:hasCookingState mecha:Cooked`.
   - It resides in a persistent world setup (`mecha:MechaWorld` of type `ue4:UWorld` and `mecha:MechaLevel` of type `ue4:ULevel`) which tracks packaging status `mecha:WasmReady` (`ue4:WasmPackagingTypestate`).
   - Replication is managed by `mecha:MechaNetworkingHandler` (`ue4:UNetworkingSubsystem`).
3. **Execution Validation:**
   - Running the validation tool harness `validate_ontology.sh` with the Mecha scenario loaded produces an error-free run with exit code `0`, confirming the schema is fully correct and passes both SHACL constraints and custom rules.
...
- We assumed that `/Users/sac/rocket-craft/ggen-validation-tests/core.ttl` is the correct test file containing the Mecha scenario.
- We did not compile or run Playwright visual tests as this is an ontology validation check.

## 4. Conclusion

The Mecha Player Character Scenario defined in Tier 4 of `TEST_INFRA.md` is fully implemented and correct. The ontologies compile and validate cleanly under `validate_ontology.sh`, and the validation rules correctly detect all classes of structural, type, and semantic defects.
...
1. Run the rules test suite:
   ...
   ./verify_all_rules.sh
   ```
2. Manually test the Mecha scenario against the production pack:
   ```bash
...
- **Assumption challenged**: The standalone file `/Users/sac/rocket-craft/ggen-validation-tests/mecha_character.ttl` is the source of truth for the scenario.
- **Attack scenario**: The category of `mecha:MoveForwardPinIn` was marked as `"float"` instead of `"exec"` in `mecha_character.ttl`. If parsed directly, this would violate `RuleE` (Exec vs. Data Pin Separation).
- **Blast radius**: The standalone file `mecha_character.ttl` would fail validation if synced independently, but the test harness uses `core.ttl` where it was correctly set to `"exec"`.
- **Mitigation**: Update `mecha_character.ttl` to keep it synchronized with the correct `core.ttl` definition.
