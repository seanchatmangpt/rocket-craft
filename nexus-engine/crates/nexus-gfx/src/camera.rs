...
impl Camera {
    ...
    pub fn view_matrix(&self) -> Mat4 {
        self.transform.to_matrix().try_inverse()
    ...
    pub fn fov_y_degrees(&self) -> f32 { self.fov_y_radians.to_degrees() }
}
...
impl Aabb {
    ...
    pub fn half_extents(&self) -> Vec3 { (self.max - self.min) / 2.0 }
    ...
    pub fn transform(&self, t: &Transform) -> Aabb {
        ...
        let transformed: Vec<Vec3> = corners.iter()
            ...
            .collect();
        let min = Vec3::new(
            transformed.iter().map(|v| v.x).fold(f32::INFINITY, f32::min),
            transformed.iter().map(|v| v.y).fold(f32::INFINITY, f32::min),
            transformed.iter().map(|v| v.z).fold(f32::INFINITY, f32::min),
        );
        let max = Vec3::new(
            transformed.iter().map(|v| v.x).fold(f32::NEG_INFINITY, f32::max),
            transformed.iter().map(|v| v.y).fold(f32::NEG_INFINITY, f32::max),
            transformed.iter().map(|v| v.z).fold(f32::NEG_INFINITY, f32::max),
        );
...
impl Frustum {
    pub fn from_view_projection(vp: &Mat4) -> Self {
        ...
        Frustum { planes: [
            // Left plane
            Vec4::new(m[(3,0)] + m[(0,0)], m[(3,1)] + m[(0,1)], m[(3,2)] + m[(0,2)], m[(3,3)] + m[(0,3)]),
            // Right plane
            Vec4::new(m[(3,0)] - m[(0,0)], m[(3,1)] - m[(0,1)], m[(3,2)] - m[(0,2)], m[(3,3)] - m[(0,3)]),
            // Bottom
            Vec4::new(m[(3,0)] + m[(1,0)], m[(3,1)] + m[(1,1)], m[(3,2)] + m[(1,2)], m[(3,3)] + m[(1,3)]),
            // Top
            Vec4::new(m[(3,0)] - m[(1,0)], m[(3,1)] - m[(1,1)], m[(3,2)] - m[(1,2)], m[(3,3)] - m[(1,3)]),
            // Near
            Vec4::new(m[(3,0)] + m[(2,0)], m[(3,1)] + m[(2,1)], m[(3,2)] + m[(2,2)], m[(3,3)] + m[(2,3)]),
            // Far
            Vec4::new(m[(3,0)] - m[(2,0)], m[(3,1)] - m[(2,1)], m[(3,2)] - m[(2,2)], m[(3,3)] - m[(2,3)]),
        ]}
    }
