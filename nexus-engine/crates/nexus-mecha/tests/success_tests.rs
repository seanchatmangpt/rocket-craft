use nexus_mecha::builder::{CivilizationBuilder, MechBuilder, ValidationError};
use nexus_mecha::mech_primitives::{
    Armor, Earth, Frame, Joint, Mars, Mobility, Power, RotationLimits, Sensor, UtilitySystem,
    Venus, Walking, Weapon, AABB,
};
use nexus_mecha::preservation::{
    MechaPreservationManager, PreservationArtifact, PreservationLayer,
};
use nexus_mecha::simulation::{ExperiencePhase, MechaNexusSimulation, SimulationInterface};
...
fn test_planetary_incompatibility() {
    use nexus_mecha::builder::PlanetMechCompatibility;
    use nexus_mecha::mech_primitives::{Earth, Mars, Venus};
...
fn test_preservation_success() {
    use nexus_mecha::mech_primitives::FlashGames;

    let manager = MechaPreservationManager::new();
    ...
    let artifact = PreservationArtifact {
        ...
        domain: FlashGames,
        name: "Mecha Battle Royale".to_string(),
        original_url: Some("http://example.com/mecha".to_string()),
        metadata,
    ...
    assert_eq!(retrieved.name, "Mecha Battle Royale");
...
fn test_simulation_success() {
    let mut sim = MechaNexusSimulation::new();
    let state = sim.get_state();
    assert_eq!(state.current_phase, ExperiencePhase::Explore);

    // Spawn mech
    sim.spawn_mech("GruntUnit II", "Warrior").unwrap();
    // Form civilization
    sim.form_civilization("ColonyRebels", "Mars").unwrap();
