...
#[derive(Default)]
pub struct MechaPreservationManager {
    registry: RwLock<HashMap<String, StoredPreservationArtifact>>,
}

impl MechaPreservationManager {
    pub fn new() -> Self {
        Self {
...
impl PreservationLayer for MechaPreservationManager {
    fn preserve_artifact<Dom: PreservationDomainCategory + OntologyName>(
        &self,
...
mod tests {
    ...
    fn mgr() -> MechaPreservationManager {
        MechaPreservationManager::new()
    }
    ...
    fn retrieve_after_preserve_recovers_data() {
        let m = mgr();
        m.preserve_artifact(arcades_artifact("a2", b"mecha data")).unwrap();
        let art: PreservationArtifact<Arcades> = m.retrieve_artifact("a2").unwrap();
        assert_eq!(art.assets_blob, b"mecha data");
