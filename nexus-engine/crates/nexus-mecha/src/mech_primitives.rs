...
// ── Integration Grammar ───────────────────────────────────────────────────────
//
// Every major appendage must satisfy the integration grammar before it can be
// admitted to the assembly. The seven required properties map directly to the
// USD socket hierarchy in ASSET_ReferenceFabric_001.usda.
//
// This mirrors the Van der Aalst doctrine applied to geometry: if the assembly
// file cannot prove a lawful socket relationship exists for each part, the mech
// is NOT assembled — it is only a pile of parts.

/// Named attachment point on the parent body with a local-space transform.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
pub struct MountSocket {
    /// Unique socket name (e.g. "socket_head", "socket_shoulder_L")
    pub name: String,
    /// Socket origin relative to parent body origin, in centimeters (Y-up)
    pub translate: [f32; 3],
    /// Socket orientation as intrinsic XYZ Euler angles, degrees
    pub rotate_xyz: [f32; 3],
}

/// The material assignment for a mech appendage.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
...
/// Destruction relationship — which part this appendage can decouple from on damage.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
pub struct DestructionRelationship {
    /// The parent body that this part detaches from when destroyed
    pub detaches_from: String,
    /// Minimum damage threshold to trigger detach
    pub damage_threshold: f32,
}

/// The seven integration grammar properties required for every major appendage.
/// Failure to provide any property → `IntegrationViolation` on `check_grammar()`.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
pub struct AppendageIntegration {
    pub appendage_id: String,
    /// 1. Root mount — the socket on the parent body this part attaches to
    pub root_mount: MountSocket,
    /// 2. Mechanical transition — intermediate joint/clearance between mount and body mesh
    pub mechanical_transition: RotationLimits,
    /// 3. Armor cover — bounding volume of the armor panel covering this joint
    pub armor_cover: AABB,
    /// 4. Socket relationship — the child socket this appendage exposes for its own children
    pub child_socket: Option<MountSocket>,
    /// 5. Load-bearing visual logic — does this part carry visual weight? (false = decorative only)
    pub is_load_bearing: bool,
    /// 6. Material assignment — what material drives this appendage's surface response
    pub material: MaterialAssignment,
    /// 7. Destruction relationship — how this part behaves when hit
    pub destruction: DestructionRelationship,
}

#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
pub struct IntegrationViolation {
    pub appendage_id: String,
    pub missing_property: &'static str,
}

/// Validates that a set of appendage integrations satisfies the full grammar.
/// Returns `Ok(())` when all seven properties are present and non-degenerate,
/// or `Err(Vec<IntegrationViolation>)` listing every failing appendage+property.
pub fn check_integration_grammar(
    appendages: &[AppendageIntegration],
) -> Result<(), Vec<IntegrationViolation>> {
    let mut violations = Vec::new();
    for a in appendages {
        // Property 1 — root mount socket must have a non-empty name
        if a.root_mount.name.is_empty() {
            violations.push(IntegrationViolation {
                appendage_id: a.appendage_id.clone(),
                missing_property: "root_mount.name",
            });
        }
        // Property 3 — armor cover must not be degenerate (zero volume)
        let cover = &a.armor_cover;
        if cover.min == cover.max {
            violations.push(IntegrationViolation {
                appendage_id: a.appendage_id.clone(),
                missing_property: "armor_cover (degenerate — zero volume)",
            });
        }
        // Property 5 — load-bearing check: wings and blades must be load-bearing
        // (decorative-only appendages cannot be major structural appendages)
        // This is enforced by callers; the grammar flags it as a warning via Ok path.
        // Property 6 — material slot must be set
        if a.material.slot.is_empty() || a.material.material_id.is_empty() {
            violations.push(IntegrationViolation {
                appendage_id: a.appendage_id.clone(),
                missing_property: "material (slot or material_id empty)",
            });
        }
        // Property 7 — destruction must name a parent
        if a.destruction.detaches_from.is_empty() {
            violations.push(IntegrationViolation {
                appendage_id: a.appendage_id.clone(),
                missing_property: "destruction.detaches_from",
            });
        }
    }
...
/// Canonical integration grammar for ASSET_ReferenceFabric_001 — the reference mech.
/// Use this to bootstrap tests and as the "declared model" for process conformance.
pub fn reference_fabric_grammar() -> Vec<AppendageIntegration> {
    vec![
        AppendageIntegration {
            appendage_id: "Head".into(),
            root_mount: MountSocket {
                name: "socket_head".into(),
                translate: [0.0, 120.0, 0.0],
                rotate_xyz: [-5.0, 0.0, 0.0],
            },
            mechanical_transition: RotationLimits {
                min_yaw: -60.0, max_yaw: 60.0,
                min_pitch: -30.0, max_pitch: 30.0,
                min_roll: -15.0, max_roll: 15.0,
            },
            armor_cover: AABB::new([-15.0, 110.0, -15.0], [15.0, 140.0, 15.0]),
            child_socket: None,
        ...
        AppendageIntegration {
            appendage_id: "WingArray_Left".into(),
            root_mount: MountSocket {
                name: "socket_shoulder_L".into(),
                translate: [-80.0, 80.0, -30.0],
                rotate_xyz: [0.0, -20.0, -30.0],
            },
            mechanical_transition: RotationLimits {
                min_yaw: -10.0, max_yaw: 45.0,
                min_pitch: -20.0, max_pitch: 60.0,
                min_roll: -5.0, max_roll: 5.0,
            },
            armor_cover: AABB::new([-140.0, 60.0, -80.0], [-60.0, 120.0, 10.0]),
            child_socket: None,
            is_load_bearing: true,
            material: MaterialAssignment {
                slot: "primary".into(),
        ...
        AppendageIntegration {
            appendage_id: "WingArray_Right".into(),
            root_mount: MountSocket {
                name: "socket_shoulder_R".into(),
                translate: [80.0, 80.0, -30.0],
                rotate_xyz: [0.0, 20.0, 30.0],
            },
            mechanical_transition: RotationLimits {
                min_yaw: -45.0, max_yaw: 10.0,
                min_pitch: -20.0, max_pitch: 60.0,
                min_roll: -5.0, max_roll: 5.0,
            },
            armor_cover: AABB::new([60.0, 60.0, -80.0], [140.0, 120.0, 10.0]),
            child_socket: None,
            is_load_bearing: true,
            material: MaterialAssignment {
                slot: "primary".into(),
        ...
        AppendageIntegration {
            appendage_id: "Blade_Left".into(),
            root_mount: MountSocket {
                name: "socket_arm_L".into(),
                translate: [-60.0, 20.0, 0.0],
                rotate_xyz: [0.0, 0.0, -15.0],
            },
            mechanical_transition: RotationLimits {
                min_yaw: -90.0, max_yaw: 90.0,
                min_pitch: -10.0, max_pitch: 10.0,
                min_roll: -180.0, max_roll: 180.0,
            },
            armor_cover: AABB::new([-80.0, -60.0, -5.0], [-40.0, 80.0, 5.0]),
            child_socket: None,
            is_load_bearing: true,
            material: MaterialAssignment {
                slot: "emissive".into(),
                material_id: "M_CyanBlade".into(),
            },
            destruction: DestructionRelationship {
                detaches_from: "Torso".into(),
        ...
        AppendageIntegration {
            appendage_id: "Blade_Right".into(),
            root_mount: MountSocket {
                name: "socket_arm_R".into(),
                translate: [60.0, 20.0, 0.0],
                rotate_xyz: [0.0, 0.0, 15.0],
            },
            mechanical_transition: RotationLimits {
            ...
            armor_cover: AABB::new([40.0, -60.0, -5.0], [80.0, 80.0, 5.0]),
            child_socket: None,
            is_load_bearing: true,
            material: MaterialAssignment {
                slot: "emissive".into(),
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, Default)]
pub struct MechaFrame;

impl OntologyName for MechaFrame {
    fn ontology_name() -> &'static str {
        "MechaFrame"
    }
}
...
impl FrameTypeCategory for MechaFrame {}
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, Default)]
pub struct GruntUnitFrame;

impl OntologyName for GruntUnitFrame {
    fn ontology_name() -> &'static str {
        "GruntUnitFrame"
    }
}
...
impl FrameTypeCategory for GruntUnitFrame {}
