...
pub trait ExperiencePhaseTrait {
    ...
    fn run(&self, sim: &mut MechaNexusSimulation) -> Result<()>;
...
// --- MechaNexusSimulation ---

pub struct MechaNexusSimulation {
...
impl Default for MechaNexusSimulation {
    fn default() -> Self {
        Self::new()
    }
}

impl SimulationInterface for MechaNexusSimulation {
    fn step_phase(&mut self) -> Result<ExperiencePhase> {
...
mod tests {
    ...
    fn sim() -> MechaNexusSimulation {
        MechaNexusSimulation::new()
    }
    ...
    fn spawn_valid_mech_succeeds() {
        let mut s = sim();
        s.spawn_mech("Nu Mecha", "Warrior").unwrap();
