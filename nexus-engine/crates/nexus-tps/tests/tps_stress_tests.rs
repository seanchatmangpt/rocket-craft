use nexus_tps::{
    assemble_mech, branchless_clamp01, branchless_lerp, JidokaHalt, PartSlot, PartStateVector,
};
...
fn test_branchless_clamp01_stability() {
    ...
    assert!(
        res_pos_inf.is_nan(),
        "Expected branchless_clamp01(INFINITY) to be NaN due to INFINITY * 0.0"
    );
    assert!(
        res_neg_inf.is_nan(),
        "Expected branchless_clamp01(-INFINITY) to be NaN due to NEG_INFINITY * 0.0"
    );
    assert!(
        res_nan.is_nan(),
        "Expected branchless_clamp01(NAN) to be NaN"
    );
}
...
fn test_random_assembly_stress() {
    ...
    for _ in 0..iterations {
        ...
        .enumerate()
        {
            ...
            let armor = if rng.next_f32() < 0.95 {
                rng.next_f32()
            } else {
                rng.next_f32_range(-0.5, 1.5)
            };
            let joint = if rng.next_f32() < 0.9 {
                shared_joint
            } else {
            ...
            let mass = if rng.next_f32() < 0.95 {
                rng.next_f32()
            } else {
                rng.next_f32_range(-0.5, 1.5)
            };
            let weapon = if rng.next_f32() < 0.95 {
                rng.next_f32()
            } else {
                rng.next_f32_range(-0.5, 1.5)
            };
            let motion = if rng.next_f32() < 0.95 {
                rng.next_f32_range(0.0, 100.0)
            } else {
                rng.next_f32_range(-10.0, 120.0)
            };

            vectors[idx] = PartStateVector {
                civilization_id: if rng.next_f32() < 0.9 {
                    shared_civ
                } else {
                    (rng.next_u32() % 100) as u16
                },
                frame_id: if rng.next_f32() < 0.9 {
                    shared_frame
                } else {
        ...
        match assemble_mech(&vectors) {
            Ok(receipt) => {
                ...
                assert!(
                    receipt.total_mass <= receipt.load_capacity,
                    "Total mass {} exceeds capacity {}",
                    receipt.total_mass,
                    receipt.load_capacity
                );
            }
