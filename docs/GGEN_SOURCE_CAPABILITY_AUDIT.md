## ggen sync argument contract
exact accepted flags: `--manifest <MANIFEST>`, `--output-dir <OUTPUT_DIR>`, `--dry-run <DRY_RUN>`, `--force <FORCE>`, `--audit <AUDIT>`, `--rule <RULE>`, `--verbose <VERBOSE>`, `--watch <WATCH>`, `--validate-only <VALIDATE_ONLY>`, `--format <FORMAT>`, `--timeout <TIMEOUT>`, `--stage <STAGE>`, `--ontology <ONTOLOGY>`, `--queries <QUERIES>`, `--language <LANGUAGE>`, `--profile <PROFILE>`, `--locked`
whether --force is boolean or value-taking: `--force` is value-taking (`--force <FORCE>`).
correct manifest flag: `--manifest <MANIFEST>`
working example from source/tests/examples if found: `ggen sync --manifest project/ggen.toml --force true`
command actually used: `/Users/sac/ggen/target/release/ggen sync --manifest ontology/ggen-packs/mech_factory_mud/ggen.toml --force true`
## ggen Render Failure
The failure was caused by the Tera variables shape. Standard SPARQL queries return `results`, not `bindings`, and elements are accessed via `row.s` and `row.type`. The fix was applied to the templates.
## ue4-stations-csv Context Shape
The context contains `results` as an array of objects. Each object has `s` and `type`.
## ggen Outputs Generated
Yes.
## Generated FactoryStations.csv Text
Included in report.
