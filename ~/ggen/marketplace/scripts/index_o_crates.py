...
ONTOLOGY_TTL = os.path.join(MARKETPLACE_ROOT, "ontology.ttl")

def calculate_blake3(path):
    h = hashlib.blake3()
...
def index_crates():
    ...
    for root, dirs, files in os.walk(CATALOGUE_ROOT):
        # We treat top-level directories under CATALOGUE_ROOT as distinct O-Crates
        ...
        if os.path.dirname(rel_path) == "":
            ...
            crate_hash = hashlib.blake3()
            for f_path in sorted(all_files):
                crate_hash.update(calculate_blake3(f_path).encode())
            ...
            crate_meta = {
                ...
                "queryCount": rq_count,
                "blake3Hash": crate_hash.hexdigest(),
